  ...
  <script>
    ...
    function renderMarketplaces() {
      ...
      const renderSupplyRows = (rows, history = false) => {
        ...
        return `<div class="op-list">${rows.map((row) => {
          ...
          return `<div class="card report-row marketplace-supply-card"><div><b>${escapeHtml(row.marketplace === "wildberries" ? "Wildberries" : "Ozon")} · ${escapeHtml(row.external_supply_id)}</b><span>${escapeHtml(row.destination_name || "Направление не указано")} · ${escapeHtml(marketplaceQuantity(itemCount))} поз. · ${escapeHtml(marketplaceQuantity(row.total_quantity))} шт.${row.unmatched_count ? `<br><span class="critical-text">Не сопоставлено: ${escapeHtml(marketplaceQuantity(row.unmatched_count))}</span>` : ""}</span></div><div class="supply-actions"><span class="status-chip ${["SHORTAGE","SYNC_ERROR","CANCELLED","EXTERNAL_DRAFT"].includes(status) ? "warn" : ""}">${escapeHtml(supplyStatusLabels[status] || status)}</span>${action}</div></div>`;
        }).join("")}</div>`;
      };
      ...
      const warehouseShipmentsBlock = warehouseShipments.length ? `<div class="op-list">${warehouseShipments.map((row) => `<div class="card report-row"><div><b>${escapeHtml(row.number)}</b><span>${escapeHtml(row.marketplace === "wildberries" ? "Wildberries" : "Ozon")} · ${escapeHtml(row.destination_name || "Направление не указано")}<br>${escapeHtml(marketplaceQuantity(row.total_quantity))} шт. · резерв ${escapeHtml(marketplaceQuantity(row.reserved_quantity))} · отобрано ${escapeHtml(marketplaceQuantity(row.picked_quantity))}</span></div><span class="status-chip">${escapeHtml(row.status)}</span></div>`).join("")}</div>` : itemEmpty("Внутренних складских отгрузок пока нет.");
    ...
    function renderAnalyticsOverviewFromApi(hubHead) {
      ...
      const suppliesBlock = supplies.length
        ? `<div class="analytics-supply-list">${supplies.slice(0, 8).map((row) => { const status = String(row.canonical_status || row.status || "UNKNOWN"); return `<div class="analytics-supply-row"><div class="analytics-supply-copy"><b>${escapeHtml(row.marketplace === "wildberries" ? "Wildberries" : "Ozon")} · ${escapeHtml(row.external_supply_id || row.number || "без номера")}</b><span>${escapeHtml(row.destination_name || "направление не указано")}${row.total_quantity !== null && row.total_quantity !== undefined ? ` · ${escapeHtml(marketplaceQuantity(row.total_quantity))} шт.` : ""}${row.unmatched_count ? ` · не сопоставлено ${escapeHtml(marketplaceQuantity(row.unmatched_count))}` : ""}</span></div><span class="status-chip ${criticalSupplyStatuses.has(status) ? "warn" : "gray"}">${escapeHtml(supplyStatusLabels[status] || status)}</span></div>`; }).join("")}</div>`
    ...
    function renderAnalyticsHub() {
      ...
      function renderSuppliesPage() {
        ...
        const rows = selected.map((row) => `<tr><td>${escapeHtml(String(row.marketplace || row.provider || "—").toLowerCase().includes("wild") ? "Wildberries" : "Ozon")}</td><td>${escapeHtml(row.external_supply_id || row.number || row.name || "—")}</td><td>${escapeHtml(row.canonical_status || row.status_label || row.status || "—")}</td><td>${escapeHtml(row.destination_name || row.destination || row.warehouse_name || "—")}</td><td>${row.total_quantity !== undefined && row.total_quantity !== null ? escapeHtml(fmt(row.total_quantity)) : row.quantity !== undefined ? escapeHtml(fmt(row.quantity)) : "—"}</td><td>${escapeHtml(row.planned_at || row.timeslot_from || "—")}</td><td>${escapeHtml(row.last_synced_at || row.updated_at || row.date || "—")}</td></tr>`);
