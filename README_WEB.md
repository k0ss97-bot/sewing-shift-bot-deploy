...
В серверной конфигурации бот может работать дочерним процессом веб-сервиса и
использовать тот же `DB_DIR`. После остановки прежнего polling-процесса создайте
маркер `$DB_DIR/bot.enabled` с правами только для владельца и перезапустите
`sewing-web.service`. Пока маркера нет, сайт запускается без Telegram-бота. Для
этого режима в серверном environment обязательно должен быть `BOT_TOKEN`;
дочерний процесс автоматически получает `MINIAPP_ENABLED=0`, поэтому второй
...
Для разового полного перехода с работающего бота добавьте `--full`: вместе со
сменами будут объединены производственные задания, раскрой, складские остатки,
движения и журнал изменений. Сначала обязательно выполните dry-run без
`--apply`, а после применения снова запустите `database.init_db()`, чтобы
создать актуальные партии остатков и события прослеживаемости для старых строк.
