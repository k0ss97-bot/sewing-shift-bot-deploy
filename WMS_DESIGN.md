...
Доступ к ТСД не является отдельной основной должностью. Администратор включает
дополнительное право `can_access_wms` выбранным действующим сотрудникам (например,
части упаковщиков); администраторы имеют доступ автоматически.
