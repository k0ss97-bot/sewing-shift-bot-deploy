    ...
    function makeSearchable(select) {
      ...
      function positionDropdown() {
        // The dropdown stays inside its positioned wrapper. This avoids
        // runtime style attributes and keeps the strict CSP enforceable.
      }
    ...
    const appStateCacheMaxBytes = 3_500_000;
    ...
    const state = {
      ...
      marketplaceFilterSnapshot: null,
      ...
      adminHomeEmployee: "",
      planningModule: "mrp",
      planningOverview: {loading: false, loaded: false, error: "", payload: null},
      planningResult: null,
      ...
      wmsShipmentTaskActiveAllocationId: "",
    ...
    if (!state.marketplaceFilters || typeof state.marketplaceFilters !== "object") {
      state.marketplaceFilters = {onlyProblems: false, inStockOnly: false, orderStatus: "all"};
    }
    state.marketplaceFilters = {
      onlyProblems: Boolean(state.marketplaceFilters.onlyProblems),
      inStockOnly: Boolean(state.marketplaceFilters.inStockOnly),
      orderStatus: String(state.marketplaceFilters.orderStatus || "all").slice(0, 80),
    };
    if (!["active", "history"].includes(state.marketplaceSupplyView)) state.marketplaceSupplyView = "active";
    if (!["auto", "comfortable", "compact"].includes(state.displayDensity)) state.displayDensity = "auto";
    if (!["all", "ozon", "wildberries"].includes(state.marketplaceProvider)) state.marketplaceProvider = "all";
    if (!["warehouses", "products", "list"].includes(state.marketplaceOrderView)) state.marketplaceOrderView = "warehouses";
    ...
    if (!["mrp", "capacity", "oee", "unit_economics", "forecast", "slotting"].includes(state.planningModule)) state.planningModule = "mrp";
    ...
    function renderBottomNav() {
      ...
      if (state.workspace === "warehouse") {
        ...
        applyCountClass(bottomNav, "nav-count", wmsItems.length);
      ...
      if (state.workspace === "marketplaces") {
        ...
        applyCountClass(bottomNav, "nav-count", marketplaceItems.length);
      ...
      applyCountClass(bottomNav, "nav-count", items.length);
    ...
    function renderTopTabs() {
      ...
      applyCountClass(topTabs, "tab-count", tabs.length || 1);
    ...
    function renderPlanFactCards(entity) {
      ...
      return `
        <div class="kpi-grid">
          <div class="card kpi"><div class="kpi-top"><span>План</span><div class="kpi-ico">${uiIcon("target")}</div></div><strong>${escapeHtml(entity.plan_text || "0")}</strong><span>Плановое количество</span><div class="progress"><progress max="100" value="0"></progress></div></div>
          <div class="card kpi good"><div class="kpi-top"><span>Факт</span><div class="kpi-ico">${uiIcon("quality")}</div></div><strong>${escapeHtml(entity.fact_text || "0")}</strong><span>Сделано по заданиям</span><div class="progress sage"><progress max="100" value="${factPercent}"></progress></div></div>
        </div>
      `;
    }
        ...
        <div class="operations-center">
          ...
          <div class="operations-layout">
            <div class="operations-panel">
              ...
              <div class="progress sage"><progress max="100" value="${planPercent}"></progress></div>
              ...
              <div class="operations-panel-head section-spacer-23"><b>Производственные этапы</b><span>WIP и загрузка</span></div>
              <div class="stage-stack">${stages.length ? stages.map((stage) => {
                ...
                return `<div class="stage-row"><b>${escapeHtml(stage.stage)}</b><div class="progress ${delayed ? "" : "sage"}"><progress max="100" value="${percent}"></progress></div><span class="status-chip ${delayed ? "warn" : waiting ? "gray" : ""}">${status}</span></div>`;
            ...
            <div class="operations-panel">
              ...
              <div class="operations-panel-head section-spacer-24"><b>Оповещения</b><span>${alerts.length}</span></div>
    ...
    async function refreshAnalyticsOverview({silent = false, force = false} = {}) {
      ...
      const cached = state.analyticsOverviewCache && state.analyticsOverviewCache[request.key];
      if (!force && cached) {
        state.analyticsOverview.payload = cached;
      ...
      try {
        ...
        state.analyticsOverviewCache[request.key] = overview;
        state.analyticsOverview.loaded = true;
      } catch (error) {
        if (state.analyticsOverview.requestKey !== request.key) return;
        ...
        <div class="op-list">
          ${operations.length ? operations.map((op, index) => `
            <div class="card op-row ${index === state.selectedOperation ? "selected" : ""}" data-select-operation="${index}">
              ...
              <div class="op-meta"><b>${escapeHtml(op.operation_name)}</b><span>${escapeHtml(op.product_size || "-")} · ${escapeHtml(op.product_color || "-")}<br>${escapeHtml(op.quantity)} ${escapeHtml(op.unit)}</span><div class="progress ${Number(op.quantity || 0) > 0 ? "sage" : ""}"><progress max="100" value="${Math.min(100, Number(op.quantity || 0))}"></progress></div></div>
    ...
    function renderTaskCompletionForm(task) {
      ...
      return `
        <div class="card task-completion-card">
          ...
          <div class="form-grid section-spacer-11">
            ...
            <div class="field full ${defectVisible ? "" : "conditional-hidden"}" id="taskDefectDetails">
              <div class="form-grid">
            ...
            <div class="op-list">
              ${cuttingWorkTasks.map((task, index) => `
                <div class="card order-card ${index === state.selectedCuttingReportTask ? "selected" : ""}" data-select-cutting-report-task="${index}">
                  ...
                  <div class="progress"><progress max="100" value="${progressForTask(task)}"></progress></div>
...
${location.code}`)) return;
    ...
    function orderPageWindow(rows) {
      const pageCount = Math.max(1, Math.ceil(rows.length / ORDER_DOM_WINDOW));
      state.orderPage = Math.max(0, Math.min(Number(state.orderPage) || 0, pageCount - 1));
      const pageStart = state.orderPage * ORDER_DOM_WINDOW;
      return {
        rows: rows.slice(pageStart, pageStart + ORDER_DOM_WINDOW),
        pageStart,
        pageCount,
      };
    }

    function renderOrderPager(total, pageCount) {
      if (total <= ORDER_DOM_WINDOW) return "";
      return `<div class="button-row order-pager"><button type="button" class="small-button secondary" data-order-action="previous-page" ${state.orderPage <= 0 ? "disabled" : ""}>← Назад</button><span class="status-chip gray">Страница ${state.orderPage + 1} из ${pageCount} · показано не более ${ORDER_DOM_WINDOW}</span><button type="button" class="small-button secondary" data-order-action="next-page" ${state.orderPage + 1 >= pageCount ? "disabled" : ""}>Далее →</button></div>`;
    }
    ...
    function renderOrdersBoard() {
      ...
      const page = orderPageWindow(allTasks);
      const columns = [
        ["in_work", "В работе"],
        ["free", "Ожидают"],
      ...
      mount.innerHTML = `
        ...
        <div class="orders-board">${columns.map(([id, label]) => {
          const totalRows = allTasks.filter((task) => bucketFor(task) === id);
          const rows = page.rows.filter((task) => bucketFor(task) === id);
          return `<section class="orders-column"><div class="orders-column-head"><b>${label}</b><span>${totalRows.length}</span></div>${rows.length ? rows.map((task) => {
    ...
    function renderOrders() {
      ...
      const page = orderPageWindow(allTasks);
      const tasks = page.rows.filter((task) => task.task_kind !== "route");
      const routeRows = page.rows.filter((task) => task.task_kind === "route");
    ...
    function renderAdminIntegrations(admin) {
      ...
      const rows = runs.slice(0, 40).map((row) => `<tr><td>${escapeHtml(row.marketplace || "—")}</td><td>${escapeHtml(row.started_at || "—")}</td><td>${escapeHtml(row.finished_at || "—")}</td><td>${escapeHtml(businessStatusLabel(row.status))}</td><td>${escapeHtml(row.error_message || row.message || "—")}</td></tr>`).join("");
    ...
    function renderWmsAdminAdjustmentForm(cellMode = false) {
      ...
      return `<div class="card field-card">
        ...
        <div class="form-grid">
          ...
          ${stock ? `<div class="field full"><label>${draft.mode === "inventory" ? "Причина (необязательно)" : "Причина"}</label><textarea id="wmsAdminReason" rows="3" placeholder="${draft.mode === "inventory" ? "Можно оставить пустым" : "Обязательная причина для журнала"}">${escapeHtml(draft.reason || "")}</textarea></div>` : ""}
        </div>
    ...
    function renderWmsMapCell(location, parts, sectionStart, stockRows, gridClass = "") {
      ...
      return `<div class="wms-map-shell">
        ...
        <div class="wms-map-scroll">${zones.map((zone) => {
          ...
          const cells = zoneRows.slice().sort((a, b) => (b.parts.level - a.parts.level) || (a.parts.section - b.parts.section) || (a.parts.position - b.parts.position)).map((row) => {
            ...
            const gridClass = column <= 20 && visualRow <= 12 ? `wms-col-${column} wms-row-${visualRow}` : "";
            return renderWmsMapCell(row.location, row.parts, row.parts.section > 1 && row.parts.position === 1, stockRows, gridClass);
          });
          const safeColumns = Math.max(1, Math.min(20, maxSection * maxPosition));
          return `<section class="wms-zone-map"><div class="wms-zone-map-head"><b>Зона №${zone}</b><span>${zoneRows.length} ячеек · ${zoneQuantity} ${escapeHtml(wmsCurrentStockFilter().unit)}</span></div><div class="wms-map-grid wms-columns-${safeColumns}">${cells.join("")}</div></section>`;
        }).join("")}</div>
      </div>`;
    }
    ...
    function renderWmsShipments() {
      ...
      const activeTaskAllocation = taskAllocations.find((allocation) => String(allocation.id) === String(state.wmsShipmentTaskActiveAllocationId || "")) || null;
    ...
    async function loadWmsShipmentTask(number) {
      try {
        ...
        state.wmsShipmentTaskExpectedAllocationId = "";
        state.wmsShipmentTaskActiveAllocationId = "";
        render();
      } catch (error) {
    ...
    function marketplaceQuantity(value, fallback = "0") {
      if (value === null || value === undefined || value === "") return fallback;
      const quantity = Number(value);
      if (!Number.isFinite(quantity)) return fallback;
      return Math.round(quantity).toLocaleString("ru-RU", {maximumFractionDigits: 0});
    }

    const businessStatusLabels = {
      success: "Успешно", error: "Ошибка", failed: "Ошибка", running: "Выполняется",
      pending: "Ожидает", cancelled: "Отменён", canceled: "Отменён",
      awaiting_packaging: "Ожидает упаковку", delivering: "Доставляется",
      accepted: "Принята", handed_over: "Передана", sync_error: "Ошибка синхронизации",
      planned: "Запланирована", data_filling: "Заполнение данных", external_draft: "Черновик площадки",
      waiting_reservation: "Ожидает резерв", shortage: "Не хватает товара",
      ready_to_pick: "Готова к отбору", picking: "Идёт отбор", picked: "Отобрана",
      packing: "Идёт упаковка", documents_required: "Нужны документы",
      ready_to_handover: "Готова к передаче", accepting: "Идёт приёмка",
      partially_accepted: "Принята частично", shipped_from_production: "Отгружена с производства",
      created: "Создана", in_process: "В работе", done: "Завершена",
      unknown: "Статус не подтверждён",
    };
    function businessStatusLabel(value) {
      const raw = String(value || "").trim();
      if (!raw) return "Статус не указан";
      return businessStatusLabels[raw.toLowerCase()] || businessStatusLabels[raw.replace(/\s+/g, "_").toLowerCase()] || raw;
    }
    ...
    function diagnosticTerminationLabel(value) {
      const normalized = String(value || "").trim().toLowerCase();
      return {
        completed: "Обновление завершено", success: "Обновление завершено",
        empty: "Источник вернул пустой ответ", no_data: "Новых данных нет",
        rate_limited: "Источник временно ограничил запросы",
        max_pages: "Достигнут безопасный предел страниц",
        stopped: "Обновление остановлено", failed: "Обновление завершилось ошибкой",
        error: "Обновление завершилось ошибкой",
      }[normalized] || (normalized ? "Требуется проверка журнала" : "—");
    }

    function safePercentClass(value) {
      const bounded = Math.max(0, Math.min(100, Number(value) || 0));
      return `pct-${Math.round(bounded / 5) * 5}`;
    }

    function safeRegionTone(index) {
      return `region-tone-${Math.abs(Number(index) || 0) % 10}`;
    }

    function applyCountClass(node, prefix, count, maximum = 12) {
      if (!node) return;
      [...node.classList].filter((name) => name.startsWith(`${prefix}-`)).forEach((name) => node.classList.remove(name));
      node.classList.add(`${prefix}-${Math.max(1, Math.min(maximum, Number(count) || 1))}`);
    }
    ...
    function marketplaceProductionStockText(row) {
      if (row?.is_inactive || row?.key === "inactive") return "Не требуется";
      ...
      return `${marketplaceQuantity(row?.production_available)} шт.`;
    }
    ...
    function marketplaceProductAvatar(row, compact = false, large = false) {
      ...
      const imageSize = large ? 72 : (compact ? 44 : 52);
      return `<span class="marketplace-product-avatar${sizeClass}"><span aria-hidden="true">▦</span>${imageUrl ? `<img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(name)}" width="${imageSize}" height="${imageSize}" loading="lazy" decoding="async" referrerpolicy="no-referrer">` : ""}</span>`;
    }
    ...
    function marketplaceProductionSource(product, rootPayload, marketplace = "ozon") {
      ...
      if (product?.is_inactive || reconciliation?.is_inactive) {
        return {known: false, inactive: true, quantity: null, reason: product?.inactive_reason || reconciliation?.inactive_reason || "Неактуальный товар", reconciliation};
      }
    ...
    function marketplaceCombinedStockLevel(marketplaceQuantity, productionSource) {
      if (productionSource?.inactive) return {key: "inactive", label: "Неактуальный"};
    ...
    function renderMarketplaceDetail(products, orders, runs) {
      ...
      if (detail.kind === "group") {
        ...
        if (!group) return "";
        const groupStock = selectedStocksUsable && group.available !== null && group.available !== undefined ? `${marketplaceQuantity(group.available)} шт.` : "—";
        return `
          ...
          <div class="marketplace-color-sections">${groupRows.length ? colorGroups.map((colorGroup) => `<section class="marketplace-color-section"><div class="marketplace-color-heading"><b>${escapeHtml(colorGroup.color)}</b><span>${colorGroup.rows.length} вариантов</span></div><div class="marketplace-variant-grid">${colorGroup.rows.map((row) => `
            <button type="button" class="card marketplace-clickable marketplace-product-card" data-marketplace-product-id="${escapeHtml(row.id)}">
              ...
              <span class="marketplace-product-card-body">
                ...
                <span class="marketplace-product-commercial"><span>${escapeHtml(providerLabel)}: ${selectedStocksUsable && row.available != null ? `${escapeHtml(marketplaceQuantity(row.available))} шт.` : "—"}</span><span>Производство: ${escapeHtml(marketplaceProductionStockText(row))}</span><span>${marketplaceMoney(row.current_price)}</span></span>
              </span>
      ...
      if (detail.kind === "product") {
        ...
        const marketplaceStockQuantity = selectedStocksUsable && product.available != null ? Number(product.available || 0) : null;
        ...
        const stockAlerts = [];
        if (marketplaceStockQuantity != null && marketplaceStockQuantity <= 0) stockAlerts.push(`${providerLabel}: товар закончился`);
        else if (marketplaceStockQuantity != null && marketplaceStockQuantity < 10) stockAlerts.push(`${providerLabel}: осталось ${marketplaceQuantity(marketplaceStockQuantity)} шт.`);
        ...
        return `
          ...
          <div class="marketplace-stock-detail-summary">
            <div class="card marketplace-stock-source-card"><span>${escapeHtml(providerLabel)} · всего</span><strong>${marketplaceStockQuantity == null ? "—" : `${marketplaceQuantity(marketplaceStockQuantity)} шт.`}</strong><small>${marketplaceStockQuantity == null ? "Источник остатков не подтверждён" : `${marketplaceWarehouseRows.length} складов с данными`}</small></div>
            <div class="card marketplace-stock-source-card"><span>Производство · готовая продукция</span><strong>${productionSource.known ? `${marketplaceQuantity(productionSource.quantity)} шт.` : "—"}</strong><small>${productionSource.known ? `Всего ${marketplaceQuantity(reconciliation?.warehouse_quantity)} · резерв ${marketplaceQuantity(reconciliation?.warehouse_reserved_quantity)}` : productionSource.reason}</small></div>
          </div>
          ...
          <div class="marketplace-stock-detail-columns">
            <section class="card marketplace-stock-detail-card">
              ...
              <div class="marketplace-stock-location-list">${marketplaceWarehouseRows.length ? marketplaceWarehouseRows.map((warehouse) => { const level = marketplaceStockLevel(warehouse.quantity); return `<div class="marketplace-stock-location"><div><b>${escapeHtml(warehouse.name)}</b><span>Доступно на складе</span></div><strong>${marketplaceQuantity(warehouse.quantity)} шт.</strong><span class="status-chip marketplace-stock-level ${level.key}">${level.label}</span></div>`; }).join("") : itemEmpty(`Разбивка по складам ${providerLabel} пока не загружена.`)}</div>
            </section>
            <section class="card marketplace-stock-detail-card">
              <div class="section-title"><b>Производство · склад готовой продукции</b><span>${productionLocations.length} ячеек</span></div>
              ${productionSource.known ? `<div class="marketplace-stock-production-totals"><span>Всего <b>${marketplaceQuantity(reconciliation?.warehouse_quantity)}</b></span><span>Резерв <b>${marketplaceQuantity(reconciliation?.warehouse_reserved_quantity)}</b></span><span>Доступно <b>${marketplaceQuantity(reconciliation?.warehouse_available_quantity)}</b></span></div><div class="marketplace-stock-location-list">${productionLocations.length ? productionLocations.map((location) => { const level = marketplaceStockLevel(location.available_quantity); return `<div class="marketplace-stock-location"><div><b>Ячейка ${escapeHtml(location.code || "Не размещён")}</b><span>Всего ${marketplaceQuantity(location.quantity)} · резерв ${marketplaceQuantity(location.reserved_quantity)}</span></div><strong>${marketplaceQuantity(location.available_quantity)} шт.</strong><span class="status-chip marketplace-stock-level ${level.key}">${level.label}</span></div>`; }).join("") : itemEmpty("Товар связан с производством, но в ячейках готовой продукции остатка нет.")}</div>` : `<div class="marketplace-stock-source-unavailable"><b>${escapeHtml(productionSource.reason)}</b><span>Физический остаток не подменяется данными маркетплейса.</span></div>`}
            </section>
          </div>
          <div class="marketplace-filter-bar"><label><span>Период по товару</span><select id="marketplaceProductPeriod"><option value="7d" ${productPeriod === "7d" ? "selected" : ""}>Последние 7 дней</option><option value="30d" ${productPeriod === "30d" ? "selected" : ""}>Последние 30 дней</option><option value="all" ${productPeriod === "all" ? "selected" : ""}>Доступная история</option></select></label><div class="marketplace-period-label">${productPeriod === "all" ? `Доступная история за ${Number(selectedAnalytics.product_history_days || 31)} день` : escapeHtml(marketplacePeriodMeta(productPeriod).label)}</div></div>
    ...
    function renderMarketplaces() {
      ...
      const stocksBlock = products.length ? `<section class="marketplace-stock-section"><div class="marketplace-stock-filters"><label><span>Номенклатура</span><select data-stock-filter="product"><option value="all">Вся номенклатура</option>${groups.map((group) => `<option value="${escapeHtml(group.key)}">${escapeHtml(group.name)}</option>`).join("")}</select></label><label><span>Цвет</span><select data-stock-filter="color"><option value="all">Все цвета</option>${stockColors.map((color) => `<option value="${escapeHtml(color)}">${escapeHtml(color)}</option>`).join("")}</select></label><label><span>Размер</span><select data-stock-filter="size"><option value="all">Все размеры</option>${stockSizes.map((size) => `<option value="${escapeHtml(size)}">${escapeHtml(size)}</option>`).join("")}</select></label><label><span>Критичность</span><select data-stock-filter="criticality"><option value="all">Все состояния</option><option value="critical">Критично · на одном из складов 0</option><option value="low">Нужно пополнить · 1–9 шт.</option><option value="enough">Хватает · от 10 шт.</option><option value="unknown">Нет связи с производством</option><option value="inactive">Неактуальные</option></select></label></div><div class="section-title"><b>Остатки по товарам</b><span data-stock-visible-count>${products.length}</span></div><div class="marketplace-stock-grid">${products.map((row) => { const marketplaceStockQuantity = Number(row.available || 0); const productionSource = marketplaceProductionSource(row, payload, stockMarketplace); const level = marketplaceCombinedStockLevel(marketplaceStockQuantity, productionSource); const productionText = productionSource.known ? `${marketplaceQuantity(productionSource.quantity)} шт.` : productionSource.reason; return `<button type="button" class="card marketplace-clickable marketplace-stock-card" data-marketplace-product-id="${escapeHtml(row.id)}" data-stock-product="${escapeHtml(row.group_key || "other")}" data-stock-color="${escapeHtml(String(row.color || ""))}" data-stock-size="${escapeHtml(String(row.size || ""))}" data-stock-criticality="${level.key}">${marketplaceProductAvatar(row)}<span class="marketplace-stock-card-body"><b>${escapeHtml(row.name || `Товар ${stockMarketplaceLabel}`)}</b><small>${escapeHtml(row.group_name || "Прочие товары")} · ${escapeHtml(row.color || "Цвет не указан")} · ${escapeHtml(row.size || "Размер не указан")} · Артикул: ${escapeHtml(row.offer_id || "—")}</small><span class="marketplace-stock-sources"><span><small>${escapeHtml(stockMarketplaceLabel)}</small><b>${marketplaceQuantity(marketplaceStockQuantity)} шт.</b></span><span class="${productionSource.known ? "" : "unknown"}"><small>Производство</small><b>${escapeHtml(productionText)}</b></span></span></span><span class="marketplace-stock-result"><span class="status-chip marketplace-stock-level ${level.key}">${level.label}</span><small>Открыть ›</small></span></button>`; }).join("")}</div><div class="marketplace-stock-empty">${itemEmpty("По выбранным фильтрам товаров нет.")}</div></section>` : itemEmpty("Остатки ещё не загружены.");
      ...
      const orderList = `<div class="op-list">${orderAnalytics.orders.map((row) => `<button type="button" class="card report-row marketplace-clickable marketplace-order-list-row" data-marketplace-order-id="${escapeHtml(row.id)}"><div><b>${escapeHtml(row.posting_number || row.external_order_id)}</b><span>${escapeHtml(row.warehouse_name)} · ${escapeHtml(row.quantity)} шт.<br>${escapeHtml(row.shipment_date || "Срок не указан")}</span></div><div class="marketplace-order-list-metrics"><b>${escapeHtml(marketplaceOrderAmount(row.amount, row.amount_available, row.amount_partial))}</b><span class="status-chip ${row.status && !["delivering", "awaiting_packaging"].includes(row.status) ? "warn" : "gray"}">${escapeHtml(businessStatusLabel(row.status))} ›</span></div></button>`).join("")}</div>`;
      ...
      const warehouseShipmentsBlock = warehouseShipments.length ? `<div class="op-list">${warehouseShipments.map((row) => `<div class="card report-row"><div><b>${escapeHtml(row.number)}</b><span>${escapeHtml(row.marketplace === "wildberries" ? "Wildberries" : "Ozon")} · ${escapeHtml(row.destination_name || "Направление не указано")}<br>${escapeHtml(marketplaceQuantity(row.total_quantity))} шт. · резерв ${escapeHtml(marketplaceQuantity(row.reserved_quantity))} · отобрано ${escapeHtml(marketplaceQuantity(row.picked_quantity))}</span></div><span class="status-chip">${escapeHtml(businessStatusLabel(row.status))}</span></div>`).join("")}</div>` : itemEmpty("Внутренних складских отгрузок пока нет.");
      const topProducts = isWildberries && !wbStocksUsable ? [] : [...products].sort((a, b) => Number(b.available || 0) - Number(a.available || 0)).slice(0, 5);
      const topProductsBlock = topProducts.length ? `<div class="marketplace-mini-list">${topProducts.map((row) => `<div class="marketplace-mini-row"><span>${escapeHtml(row.name || row.offer_id || "Товар")}</span><b>${escapeHtml(marketplaceQuantity(row.available))} шт.</b></div>`).join("")}</div>` : itemEmpty("После синхронизации здесь появятся товары-лидеры.");
      const recentOrdersBlock = verifiedOrdersAvailable && verifiedOrders.length ? `<table class="marketplace-table"><thead><tr><th>Заказ</th><th>Товар</th><th>Статус</th></tr></thead><tbody>${verifiedOrders.slice(0, 5).map((row) => `<tr><td>${escapeHtml(row.posting_number || row.external_order_id || "—")}</td><td>${escapeHtml(row.external_order_id || "—")}</td><td>${escapeHtml(businessStatusLabel(row.status))}</td></tr>`).join("")}</tbody></table>` : itemEmpty(isWildberries && !verifiedOrdersAvailable ? "Период или источник заказов не подтверждён API Wildberries." : "За выбранный период заказов нет.");
      ...
      const chartBars = (values, wb = false) => { const max = Math.max(1, ...values.map((value) => Number(value || 0))); return values.map((value) => `<i class="marketplace-chart-bar ${wb ? "wb" : ""} ${safePercentClass(Math.max(8, (Number(value || 0) / max) * 100))}" title="${escapeHtml(value || 0)}"></i>`).join(""); };
      ...
      const qualityIntro = quality.state === "disabled"
        ...
        : quality.state === "unavailable"
          ...
          : "";
      const qualityDatasetCards = qualityDatasets.length ? qualityDatasets.map((row) => `<div class="card field-card"><div class="section-title"><b>${escapeHtml(qualityDatasetLabel[row.dataset] || "Набор данных")}</b><span class="status-chip ${qualityChipClass(row.status)}">${escapeHtml(qualityStateLabel[row.status] || "требуется проверка")}</span></div><div class="marketplace-mini-list"><div class="marketplace-mini-row"><span>Последнее успешное обновление</span><b>${qualityMoment(row.last_usable_at || row.last_success_at || (row.status === "success" ? row.finished_at : ""))}</b></div><div class="marketplace-mini-row"><span>Актуальность</span><b>${escapeHtml(qualityStateLabel[row.freshness] || "неизвестно")}</b></div><div class="marketplace-mini-row"><span>Строки: получено / уникально / ожидалось</span><b>${escapeHtml(row.received_count == null ? "—" : row.received_count)} / ${escapeHtml(row.unique_count == null ? "—" : row.unique_count)} / ${escapeHtml(row.expected_count == null ? "—" : row.expected_count)}</b></div><div class="marketplace-mini-row"><span>Страницы / повторные попытки</span><b>${escapeHtml(row.page_count == null ? "—" : row.page_count)} / ${escapeHtml(row.retry_count == null ? "—" : row.retry_count)}</b></div><div class="marketplace-mini-row"><span>Результат</span><b>${escapeHtml(diagnosticTerminationLabel(row.termination_reason))}</b></div></div>${row.error_summary ? `<div class="task-note"><b>Диагностика</b><br>${escapeHtml(row.error_summary)}</div>` : ""}</div>`).join("") : `<div class="card field-card">${itemEmpty("Обновления данных ещё не запускались.")}</div>`;
      const capabilityLabels = {roles:"Права доступа Ozon",catalog:"Каталог товаров",prices:"Цены",stocks:"Остатки",supplies:"Поставки",orders:"Заказы",finance:"Финансы",advertising:"Реклама",funnel:"Воронка продаж"};
      const capabilityRows = qualityCapabilities.length ? qualityCapabilities.map((row) => `<tr><td>${escapeHtml(capabilityLabels[row.capability] || "Источник данных")}</td><td><span class="status-chip ${qualityChipClass(row.status)}">${escapeHtml(qualityStateLabel[row.status] || "требуется проверка")}</span></td><td>${qualityMoment(row.checked_at)}</td><td>${escapeHtml(row.safe_message || "—")}</td></tr>`).join("") : `<tr><td colspan="4">Источники данных ещё не проверены.</td></tr>`;
    ...
    async function refreshAdminPlanning({silent = false} = {}) {
      if (!state.data || !state.data.is_admin || state.planningOverview.loading) return;
      state.planningOverview.loading = true;
      state.planningOverview.error = "";
      if (!silent) render();
      try {
        state.planningOverview.payload = await api("/api/admin/planning/overview");
        state.planningOverview.loaded = true;
      } catch (error) {
        state.planningOverview.error = error.apiMessage || error.message || "Не удалось загрузить контур планирования.";
      } finally {
        state.planningOverview.loading = false;
        if (state.screen === "admin" && state.adminSection === "planning") render();
      }
    }

    function planningInputNumber(id, fallback = 0) {
      const node = document.getElementById(id);
      const value = Number(node ? node.value : fallback);
      return Number.isFinite(value) ? value : fallback;
    }

    function planningPayloadFromForm() {
      const module = state.planningModule;
      if (module === "mrp") return {
        module,
        demands: [{product_master_id: planningInputNumber("planningMrpProductId"), quantity: planningInputNumber("planningMrpDemand")}],
        bom_lines: [{
          product_master_id: planningInputNumber("planningMrpProductId"),
          component_type: "material",
          component_article: document.getElementById("planningMrpArticle")?.value || "",
          component_name: document.getElementById("planningMrpName")?.value || "Ткань",
          unit: document.getElementById("planningMrpUnit")?.value || "м",
          quantity_per: planningInputNumber("planningMrpPerUnit"),
          scrap_percent: planningInputNumber("planningMrpScrap"),
        }],
        available_rows: [{
          component_type: "material",
          component_article: document.getElementById("planningMrpArticle")?.value || "",
          component_name: document.getElementById("planningMrpName")?.value || "Ткань",
          unit: document.getElementById("planningMrpUnit")?.value || "м",
          available_quantity: planningInputNumber("planningMrpAvailable"),
        }],
      };
      if (module === "capacity") return {
        module,
        jobs: [{
          job_key: document.getElementById("planningJobKey")?.value || "JOB-1",
          operation_code: document.getElementById("planningOperationCode")?.value || "SEW",
          quantity: planningInputNumber("planningJobQuantity"),
          standard_minutes_per_unit: planningInputNumber("planningStandardMinutes"),
        }],
        resources: [{
          resource_id: planningInputNumber("planningResourceId"),
          name: document.getElementById("planningResourceName")?.value || "Сотрудник",
          capacity_minutes: planningInputNumber("planningCapacityMinutes"),
          skills: {[document.getElementById("planningOperationCode")?.value || "SEW"]: planningInputNumber("planningProficiency", 1)},
        }],
      };
      if (module === "oee") return {module, inputs: {
        planned_production_minutes: planningInputNumber("planningOeePlanned"),
        unplanned_downtime_minutes: planningInputNumber("planningOeeDowntime"),
        total_units: planningInputNumber("planningOeeTotal"),
        good_units: planningInputNumber("planningOeeGood"),
        ideal_cycle_minutes: planningInputNumber("planningOeeCycle"),
      }};
      if (module === "unit_economics") return {module, inputs: {
        quantity: planningInputNumber("planningEconomicsQuantity"),
        revenue: planningInputNumber("planningEconomicsRevenue"),
        unit_cost: planningInputNumber("planningEconomicsUnitCost"),
        marketplace_fees: planningInputNumber("planningEconomicsFees"),
        logistics_cost: planningInputNumber("planningEconomicsLogistics"),
        returns_cost: planningInputNumber("planningEconomicsReturns"),
        other_variable_cost: planningInputNumber("planningEconomicsOther"),
      }};
      if (module === "forecast") {
        const history = String(document.getElementById("planningForecastHistory")?.value || "")
          .split(/[,;\s]+/).filter(Boolean).map((units) => ({units: Number(units)}));
        return {module, history, horizon_days: planningInputNumber("planningForecastHorizon"), window_days: planningInputNumber("planningForecastWindow")};
      }
      const products = String(document.getElementById("planningSlottingRows")?.value || "")
        .split("\n").map((line) => line.trim()).filter(Boolean).map((line) => {
          const [article, pickCount, pickedUnits] = line.split(/[;,]/).map((value) => value.trim());
          return {article, pick_count: Number(pickCount || 0), picked_units: Number(pickedUnits || 0)};
        });
      return {module: "slotting", products};
    }

    async function calculateAdminPlanning() {
      const actionKey = "admin-planning-calculate";
      if (!beginAction(actionKey)) return;
      try {
        state.planningResult = await api("/api/admin/planning/calculate", planningPayloadFromForm());
        render();
        showToast("Планирование", "Расчёт выполнен без изменения складских остатков.");
      } catch (error) {
        showToast("Планирование", error.apiMessage || error.message || "Расчёт не выполнен.");
      } finally {
        endAction(actionKey);
      }
    }
    ...
    function planningStatusLabel(value) {
      return ({ready: "Готово", shortage: "Дефицит", incomplete: "Не хватает данных", over_capacity: "Не хватает мощности", partial: "Частично", unavailable: "Недоступно"})[value] || String(value || "—");
    }

    function renderPlanningResult() {
      const root = state.planningResult && state.planningResult.result;
      if (!root) return itemEmpty("Заполните параметры и выполните расчёт.");
      const rows = Array.isArray(root.lines) ? root.lines : Array.isArray(root.assignments) ? root.assignments : Array.isArray(root.rows) ? root.rows : [];
      const primitives = Object.entries(root).filter(([, value]) => value == null || ["string", "number", "boolean"].includes(typeof value));
      const summary = primitives.map(([key, value]) => `<div class="detail-box"><span>${escapeHtml(key.replaceAll("_", " "))}</span><strong>${escapeHtml(key === "status" ? planningStatusLabel(value) : value == null ? "—" : value)}</strong></div>`).join("");
      const list = rows.slice(0, 50).map((row, index) => {
        const title = row.component_name || row.resource_name || row.article || row.job_key || `Строка ${index + 1}`;
        const detail = Object.entries(row).filter(([, value]) => value != null && typeof value !== "object").map(([key, value]) => `${key.replaceAll("_", " ")}: ${value}`).join(" · ");
        return `<div class="report-row"><div><b>${escapeHtml(title)}</b><span>${escapeHtml(detail)}</span></div></div>`;
      }).join("");
      const forecast = Array.isArray(root.forecast_units) ? `<div class="report-row"><div><b>Прогноз по дням</b><span>${escapeHtml(root.forecast_units.join(" · "))}</span></div></div>` : "";
      return `<div class="detail-grid">${summary}</div><div class="op-list">${list}${forecast}</div>`;
    }

    function renderPlanningForm() {
      if (state.planningModule === "mrp") return `<div class="form-grid"><div class="field"><label>ID изделия</label><input id="planningMrpProductId" type="number" min="1" value="1"></div><div class="field"><label>План выпуска, шт.</label><input id="planningMrpDemand" type="number" min="1" value="100"></div><div class="field"><label>Артикул материала</label><input id="planningMrpArticle" value="FABRIC-1"></div><div class="field"><label>Материал</label><input id="planningMrpName" value="Ткань"></div><div class="field"><label>Единица</label><input id="planningMrpUnit" value="м"></div><div class="field"><label>На изделие</label><input id="planningMrpPerUnit" type="number" min="0.000001" step="0.01" value="1.5"></div><div class="field"><label>Отход, %</label><input id="planningMrpScrap" type="number" min="0" max="100" value="5"></div><div class="field"><label>Доступно</label><input id="planningMrpAvailable" type="number" min="0" value="100"></div></div>`;
      if (state.planningModule === "capacity") return `<div class="form-grid"><div class="field"><label>Код задания</label><input id="planningJobKey" value="JOB-1"></div><div class="field"><label>Код операции</label><input id="planningOperationCode" value="SEW"></div><div class="field"><label>Количество</label><input id="planningJobQuantity" type="number" min="1" value="100"></div><div class="field"><label>Норма, мин/шт.</label><input id="planningStandardMinutes" type="number" min="0.01" step="0.01" value="5"></div><div class="field"><label>ID ресурса</label><input id="planningResourceId" type="number" min="1" value="1"></div><div class="field"><label>Сотрудник / ресурс</label><input id="planningResourceName" value="Швейный участок"></div><div class="field"><label>Доступно минут</label><input id="planningCapacityMinutes" type="number" min="1" value="480"></div><div class="field"><label>Коэффициент навыка</label><input id="planningProficiency" type="number" min="0.25" max="2" step="0.05" value="1"></div></div>`;
      if (state.planningModule === "oee") return `<div class="form-grid"><div class="field"><label>Плановое время, мин.</label><input id="planningOeePlanned" type="number" min="0" value="480"></div><div class="field"><label>Простой, мин.</label><input id="planningOeeDowntime" type="number" min="0" value="60"></div><div class="field"><label>Выпущено всего</label><input id="planningOeeTotal" type="number" min="0" value="100"></div><div class="field"><label>Годно</label><input id="planningOeeGood" type="number" min="0" value="95"></div><div class="field"><label>Идеальный цикл, мин.</label><input id="planningOeeCycle" type="number" min="0" step="0.01" value="4"></div></div>`;
      if (state.planningModule === "unit_economics") return `<div class="form-grid"><div class="field"><label>Продано, шт.</label><input id="planningEconomicsQuantity" type="number" min="0" value="100"></div><div class="field"><label>Выручка, ₽</label><input id="planningEconomicsRevenue" type="number" min="0" value="500000"></div><div class="field"><label>Себестоимость единицы, ₽</label><input id="planningEconomicsUnitCost" type="number" min="0" value="1500"></div><div class="field"><label>Комиссии, ₽</label><input id="planningEconomicsFees" type="number" min="0" value="75000"></div><div class="field"><label>Логистика, ₽</label><input id="planningEconomicsLogistics" type="number" min="0" value="30000"></div><div class="field"><label>Возвраты, ₽</label><input id="planningEconomicsReturns" type="number" min="0" value="5000"></div><div class="field"><label>Прочие расходы, ₽</label><input id="planningEconomicsOther" type="number" min="0" value="0"></div></div>`;
      if (state.planningModule === "forecast") return `<div class="form-grid"><div class="field full"><label>Продажи по дням, шт. через запятую</label><textarea id="planningForecastHistory" rows="3">10, 12, 9, 14, 15, 13, 16</textarea></div><div class="field"><label>Горизонт, дней</label><input id="planningForecastHorizon" type="number" min="1" max="366" value="7"></div><div class="field"><label>Окно средней, дней</label><input id="planningForecastWindow" type="number" min="1" value="7"></div></div>`;
      return `<div class="field full"><label>Артикул; число подборов; подобрано штук — одна строка на товар</label><textarea id="planningSlottingRows" rows="6">ARTICLE-1; 25; 120\nARTICLE-2; 10; 60</textarea></div>`;
    }

    function renderAdminPlanning() {
      const overview = state.planningOverview.payload || {};
      const counts = overview.counts || {};
      if (!state.planningOverview.loaded && !state.planningOverview.loading && !state.planningOverview.error) window.setTimeout(() => refreshAdminPlanning({silent: true}), 0);
      mainButton.textContent = "Выполнить расчёт";
      mainButton.disabled = false;
      const moduleOptions = [["mrp","MRP · потребность"],["capacity","Мощности"],["oee","OEE и качество"],["unit_economics","Себестоимость и маржа"],["forecast","Прогноз спроса"],["slotting","Размещение WMS"]];
      const readiness = overview.available ? "PostgreSQL-контур готов" : state.planningOverview.loading ? "Проверяем PostgreSQL" : overview.message || state.planningOverview.error || "Контур ещё не проверен";
      return `<div class="screen-head"><div><h2>Операционное планирование</h2><p>MRP, загрузка сотрудников, OEE, себестоимость, прогноз и оптимизация склада. Расчёты не списывают остатки и не закрывают задания.</p></div><div class="date">${escapeHtml(readiness)}</div></div>${renderAdminTabs()}<div class="kpi-grid"><div class="card kpi"><span>Активные BOM</span><strong>${escapeHtml(counts.active_bom || 0)}</strong><small>Спецификации изделий</small></div><div class="card kpi"><span>Ресурсы</span><strong>${escapeHtml(counts.resources || 0)}</strong><small>Сотрудники и участки</small></div><div class="card kpi"><span>Открытые CAPA</span><strong>${escapeHtml(counts.capa_open || 0)}</strong><small>Корректирующие действия</small></div><div class="card kpi"><span>Прогнозы</span><strong>${escapeHtml(counts.forecast_runs || 0)}</strong><small>Сохранённые расчёты</small></div></div><div class="card field-card"><div class="form-grid"><div class="field full"><label>Модуль</label><select id="planningModuleSelect">${moduleOptions.map(([id,label]) => `<option value="${id}" ${state.planningModule === id ? "selected" : ""}>${label}</option>`).join("")}</select></div></div>${renderPlanningForm()}<div class="button-row"><button class="small-button secondary" data-admin-action="planning-refresh">Проверить PostgreSQL</button><button class="small-button" data-admin-action="planning-calculate">Рассчитать</button></div></div><div class="section-title"><b>Результат расчёта</b><span>${escapeHtml(planningStatusLabel(state.planningResult?.result?.status))}</span></div><div class="card field-card">${renderPlanningResult()}</div>`;
    }
    ...
    function renderAdmin() {
      ...
      if (state.adminSection === "planning") {
        mount.innerHTML = renderAdminPlanning();
        return;
      }
    ...
    function renderProfile() {
      ...
      mount.innerHTML = `
        ...
        <div class="card field-card">
          ...
          <p class="operational-message">Автоматически: компактно для администратора на ПК, комфортно для сотрудника и мобильного устройства.</p>
          <div class="segment-row profile-density" role="group" aria-label="Плотность интерфейса">
            ${[["auto","Авто"],["comfortable","Комфортно"],["compact","Компактно"]].map(([id,label]) => `<button type="button" class="segment-button ${state.displayDensity === id ? "active" : ""}" aria-pressed="${state.displayDensity === id ? "true" : "false"}" data-profile-action="density" data-density="${id}">${label}</button>`).join("")}
          </div>
        </div>
        <div class="card field-card">
    ...
    function analyticsHubTabs() {
      ...
      return `<div class="top-tabs analytics-hub-tabs" role="tablist" aria-label="Раздел аналитики">${tabs.map(([id, label]) => `<button type="button" role="tab" aria-selected="${state.analyticsHubTab === id ? "true" : "false"}" class="${state.analyticsHubTab === id ? "active" : ""}" data-ac-page="${id}">${label}</button>`).join("")}</div>`;
    }
    ...
    function analyticsHubCombinedChart(models, metric = "money") {
      ...
      if (!dates.length) return `<div class="marketplace-chart-empty"><b>За выбранный период данных нет</b><span>${metric === "units" ? "Источники доступны, но в диапазоне нет продаж в штуках." : metric === "amount" ? "Источники доступны, но в диапазоне нет сумм заказов." : "Источники доступны, но в диапазоне нет финансовых операций."}</span></div><div class="analytics-chart-legend">${series.map((item, index) => `<span><i class="${safeRegionTone(index)}"></i>${escapeHtml(item.label)}</span>`).join("")}</div>`;
    ...
    function renderAnalyticsOverviewFromApi(hubHead) {
      ...
      const matrixBlock = !reconciliation.ok
        ? `<div class="marketplace-chart-empty"><b>Связь со складом временно недоступна</b><span>Данные не заменяются нулями. Обновите страницу после восстановления PostgreSQL.</span></div>`
        : !matrixRows.length
          ? `<div class="marketplace-chart-empty"><b>Нет сопоставленных карточек</b><span>Настройте маршруты товаров в разделе связей каталога.</span><button type="button" class="small-button secondary" data-analytics-matrix-action="links">Открыть связи</button></div>`
          : `<div class="analytics-risk-list">${matrixRows.slice(0, 10).map((row) => {
              ...
              const statusText = !routeReady ? "Нет маршрута" : !warehouseKnown ? "Склад недоступен" : available > 0 ? `${formatInteger(available)} шт.` : "Нужно произвести";
              const statusClass = !routeReady || !warehouseKnown || available <= 0 ? "warn" : "";
              const productIdentity = routeReady
    ...
    function renderAnalyticsHub() {
      ...
      const comparisonMetric = (code, providerName = marketplace) => {
        if (providerName === "production") return null;
        if (providerName === "all") {
          const rows = comparison.metrics && typeof comparison.metrics === "object" ? comparison.metrics : {};
          return rows[code] || null;
        }
        const providerRows = Array.isArray(comparison.providers) ? comparison.providers : [];
        const provider = providerRows.find((row) => row.marketplace === providerName);
        return provider && provider.metrics && typeof provider.metrics === "object" ? provider.metrics[code] || null : null;
      };
      const production = payload.production && typeof payload.production === "object" ? payload.production : {};
      const suppliesEnvelope = payload.supplies && typeof payload.supplies === "object" ? payload.supplies : {};
      ...
      const risks = Array.isArray(payload.risks) ? payload.risks : [];
      const geography = payload.geography && typeof payload.geography === "object" ? payload.geography : {};
      ...
      const comparisonLabel = (row, compact = false) => {
        if (!row || row.available !== true) return "";
        const direction = row.direction === "up" ? "up" : row.direction === "down" ? "down" : "flat";
        const arrow = direction === "up" ? "↑" : direction === "down" ? "↓" : "→";
        let text = "Без изменений";
        if (row.percent_change !== null && row.percent_change !== undefined) {
          const value = Math.abs(Number(row.percent_change)).toLocaleString("ru-RU", {maximumFractionDigits: 1});
          text = Number(row.percent_change) === 0 ? "Без изменений" : `${arrow} ${value}%`;
        } else if (row.direction === "up" && Number(row.previous) === 0) {
          text = "↑ Новый результат";
        } else if (row.direction !== "flat") {
          text = `${arrow} Изменение с нулевой базы`;
        }
        return compact ? text : `${text} к предыдущему периоду`;
      };
      const comparisonBadge = (row) => {
        const label = comparisonLabel(row);
        if (!label) return `<em class="ac-kpi-delta unavailable">Нет данных для сравнения</em>`;
        const tone = row.direction === "up" ? "up" : row.direction === "down" ? "down" : "flat";
        return `<em class="ac-kpi-delta ${tone}">${escapeHtml(label)}</em>`;
      };
      const kpi = (label, value, hint, tone = "", comparisonRow = null) => `<article class="ac-kpi ${tone}"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong><small>${escapeHtml(hint || "Показатель пока недоступен")}</small>${comparisonRow ? comparisonBadge(comparisonRow) : ""}</article>`;
      ...
      const geographyRows = (Array.isArray(geography.rows) ? geography.rows : []).filter(providerFilter);
      ...
      const salesSeries = payload.series && Array.isArray(payload.series.sales) ? payload.series.sales : [];
      const salesChartModels = (dimension) => providers.filter(providerFilter).map((provider) => {
        const key = provider.marketplace === "wildberries" ? "wildberries" : "ozon";
      ...
      function renderOverviewPage() {
        const grossSales = metric("gross_sales"), salesUnits = metric("sales_units"), orderMetric = metric("orders"), stock = metric("stock_available"), stockValue = metric("stock_retail_value");
        ...
        const cards = `<div class="ac-kpis">${kpi("Продано", safeValue(salesUnits), "Товаров за период, шт.", "", comparisonMetric("sales_units"))}${kpi("Сумма заказов", safeValue(grossSales, money), "По ценам строк заказа", "", comparisonMetric("gross_sales"))}${kpi("Заказы", safeValue(orderMetric), "За выбранный период", "", comparisonMetric("orders"))}${kpi("Остатки", safeValue(stock), "На площадках, шт.")}${kpi("Стоимость остатков", safeValue(stockValue, money), "Остаток × текущая цена")}${kpi("Производство", productionValue, "Факт / план")}</div>`;
        const providerRows = providerCards.map((row) => {
          const trend = comparisonMetric("gross_sales", row.marketplace);
          return `<tr><td><b>${escapeHtml(row.label || row.marketplace || "Площадка")}</b><br><small>${escapeHtml(row.status === "fresh" ? "Актуально" : row.status === "stale" ? "Задерживается" : "Частично")}</small></td><td>${row.gross_sales !== null && row.gross_sales !== undefined ? escapeHtml(money(row.gross_sales)) : "—"}</td><td>${trend && trend.previous !== null && trend.previous !== undefined ? escapeHtml(money(trend.previous)) : "—"}</td><td><span class="ac-table-delta ${trend && trend.direction ? escapeHtml(trend.direction) : "unavailable"}">${escapeHtml(comparisonLabel(trend, true) || "Нет данных")}</span></td></tr>`;
        });
        ...
        const supplyList = supplyRows.filter(providerFilter).slice(0, 5).map((row) => `<div class="ac-list-row"><div><b>${escapeHtml(row.external_supply_id || row.number || row.name || "Поставка")}</b><span>${escapeHtml(row.destination_name || row.destination || "Склад не указан")} · ${escapeHtml(businessStatusLabel(row.status_label || row.canonical_status || row.status))}</span></div><strong>${row.total_quantity !== undefined && row.total_quantity !== null ? `${escapeHtml(fmt(row.total_quantity))} шт.` : row.quantity !== undefined ? `${escapeHtml(fmt(row.quantity))} шт.` : ""}</strong></div>`).join("");
        const matrixList = matrixRows.slice(0, 6).map((row) => {
          const available = Number(row.warehouse_available_quantity || 0);
          const routeReady = Boolean(row.route_configured);
          const warehouseKnown = reconciliation.warehouse_available !== false;
        ...
        const matrixBody = !reconciliation.ok
          ...
          : matrixList
            ? `${matrixList}${matrixActions}`
            : `${empty("Нет сопоставленных карточек", "Настройте маршрут товара и связь с физической ячейкой.")}${matrixActions}`;
        ...
        const comparisonPeriod = comparison.period && comparison.period.label ? comparison.period.label : "Предыдущий равный период";
        const providerComparison = providerRows.length ? table(["Площадка","Сейчас","Ранее","Динамика"], providerRows, "Нет данных для сравнения площадок.") : empty("Нет подключённых площадок", "Подключите Ozon или Wildberries в настройках.");
        return `${filterbar}${cards}${sourceState}<div class="ac-grid">${panel("Продажи и финансы", period, analyticsHubCombinedChart(chartModels), "span-8")}${panel("Сравнение площадок", comparisonPeriod, providerComparison, "span-4")}${panel("Товарные риски", `${filteredRisks.length} сигналов`, riskRows || empty("Критических рисков нет", "Новых подтверждённых рисков за период не найдено."), "span-4")}${panel("Остатки → производство", `${matrixRows.length} карточек`, matrixBody, "span-4")}${panel("Поставки", `${supplyRows.filter(providerFilter).length} записей`, supplyList || empty("Поставок нет", "За выбранный период поставки не найдены."), "span-4")}${panel("Рекомендации", "На основе рисков", recommendations || empty("Рекомендаций нет", "Данных достаточно или источники пока не сформировали рекомендации."), "span-12")}</div>`;
      }
      ...
      function renderProductsPage() {
        const productLookup = new Map(products.map((row) => [`${String(row.marketplace || "ozon")}:${String(row.offer_id || row.article || row.sku || row.external_product_id || "")}`, row]));
        const reconciled = reconciliationRows.filter(providerFilter).filter((row) => !search || [row.name,row.article,row.sku,row.color,row.size,row.production_name].some((value) => String(value || "").toLowerCase().includes(search)));
        const visibleProducts = reconciled.slice(0, 500);
        const rows = visibleProducts.map((row) => {
          const source = productLookup.get(`${row.marketplace}:${row.article}`) || {};
          const stateLabel = row.is_inactive ? "Неактуален" : !row.route_configured ? "Нет связи" : row.warehouse_found ? "Связан, есть остаток" : "Связан, нужен выпуск";
          const reason = row.is_inactive ? row.inactive_reason : row.route_configured ? `${row.production_name || row.name} · ${row.production_color || row.color || "—"} · ${row.production_size || row.size || "—"}` : "Требуется проверить название, цвет и размер";
          return `<tr><td>${escapeHtml(row.marketplace === "wildberries" ? "Wildberries" : "Ozon")}</td><td>${escapeHtml(row.name || "—")}</td><td>${escapeHtml(row.article || row.sku || "—")}</td><td>${escapeHtml(row.color || "—")}</td><td>${escapeHtml(row.size || "—")}</td><td>${source.available !== null && source.available !== undefined ? fmt(source.available) : "—"}</td><td>${reconciliation.warehouse_available === false ? "—" : fmt(row.warehouse_available_quantity || 0)}</td><td><b>${escapeHtml(stateLabel)}</b><br><small>${escapeHtml(reason || "—")}</small></td></tr>`;
        });
        const action = `<div class="button-row"><button type="button" class="small-button secondary" data-analytics-matrix-action="links">Открыть связи товаров</button></div>`;
        return `<div class="ac-kpis">${kpi("Активные карточки", fmt(Math.max(0, summaryValue("products") - summaryValue("inactive"))), "В текущем каталоге")}${kpi("Связано с производством", fmt(summaryValue("route_configured")), "Есть маршрут цвета и размера")}${kpi("Неактуальные", fmt(summaryValue("inactive")), "Старше 90 дней и без продаж / сняты")}${kpi("Без маршрута", fmt(summaryValue("route_missing")), "Требуют ручной проверки")}</div><div class="ac-toolbar"><label class="ac-page-search"><span>Поиск по товару или артикулу</span><input id="analyticsSearch" value="${escapeHtml(state.analyticsSearch || "")}" placeholder="Название, артикул, цвет, размер"></label><span>${fmt(reconciled.length)} из ${fmt(reconciliationRows.filter(providerFilter).length)}</span></div><div class="ac-grid">${panel("Каталог и связь с производством", `Показано ${visibleProducts.length} из ${reconciled.length}`, `${table(["Площадка","Товар","Артикул","Цвет","Размер","На площадке","На производстве","Состояние"], rows, "Каталог площадки пока не загружен.")}${action}`, "span-12")}</div>`;
      }

      function renderInventoryPage() {
        const visibleInventory = inventoryProducts.slice(0, 300);
        const rows = visibleInventory.map((row) => `<tr><td>${escapeHtml(row.name || row.product_name || "—")}</td><td>${escapeHtml(row.offer_id || row.article || "—")}</td><td>${escapeHtml(row.size || "—")}</td><td>${escapeHtml(row.color || "—")}</td><td>${fmt(row.available)}</td><td>${row.current_price !== null && row.current_price !== undefined ? escapeHtml(money(row.current_price)) : "—"}</td><td>${row.current_price !== null && row.current_price !== undefined ? escapeHtml(money(Number(row.available) * Number(row.current_price))) : "—"}</td></tr>`);
        ...
        const inventoryComparison = providerCards.map((row) => `<tr><td>${escapeHtml(row.label || row.marketplace)}</td><td>${row.stock_available !== null && row.stock_available !== undefined ? escapeHtml(fmt(row.stock_available)) : "—"}</td><td>${row.stock_retail_value !== null && row.stock_retail_value !== undefined ? escapeHtml(money(row.stock_retail_value)) : "—"}</td></tr>`);
        return `<div class="ac-kpis">${kpi("Остатки", safeValue(metric("stock_available")), "Подтверждённые, шт.")}${kpi("Стоимость остатков", safeValue(metric("stock_retail_value"), money), "Остаток × текущая цена")}${kpi("SKU с остатком", fmt(inventoryProducts.length), "Без подмены неизвестного на ноль")}${kpi("Нет готового остатка", fmt(summaryValue("not_in_warehouse")), "Связанные с производством карточки")}</div><div class="ac-grid">${panel("Ozon / Wildberries", "Остатки в штуках и рублях", table(["Площадка","Остатки, шт.","Стоимость"], inventoryComparison, "Подтверждённые остатки площадок пока недоступны."), "span-12")}${panel("Остатки по SKU", `Показано ${visibleInventory.length} из ${inventoryProducts.length}`, table(["Товар","Артикул","Размер","Цвет","Доступно","Цена","Стоимость"], rows, "Подтверждённые остатки выбранной площадки пока недоступны."), "span-8")}${panel("Сигналы для проверки", "Без автоматического создания заданий", riskList || empty("Сигналов нет", "Недостаток товара не подтверждён или данные ещё загружаются."), "span-4")}</div>`;
      }
      ...
      function renderSuppliesPage() {
        const selected = supplyRows.filter(providerFilter).filter((row) => {
          const day = String(row.planned_at || row.timeslot_from || row.created_at_external || row.updated_at_external || row.last_synced_at || row.updated_at || "").slice(0, 10);
          return !day || !periodStart || !periodEnd || (day >= periodStart && day <= periodEnd);
        });
        const totalQuantity = selected.reduce((sum, row) => sum + Number(row.total_quantity ?? row.quantity ?? 0), 0);
        const rows = selected.map((row) => `<tr><td>${escapeHtml(String(row.marketplace || row.provider || "—").toLowerCase().includes("wild") ? "Wildberries" : "Ozon")}</td><td>${escapeHtml(row.external_supply_id || row.number || row.name || "—")}</td><td>${escapeHtml(businessStatusLabel(row.status_label || row.canonical_status || row.status))}</td><td>${escapeHtml(row.destination_name || row.destination || row.warehouse_name || "—")}</td><td>${row.total_quantity !== undefined && row.total_quantity !== null ? escapeHtml(fmt(row.total_quantity)) : row.quantity !== undefined ? escapeHtml(fmt(row.quantity)) : "—"}</td><td>${escapeHtml(row.planned_at || row.timeslot_from || "—")}</td><td>${escapeHtml(row.last_synced_at || row.updated_at || row.date || "—")}</td></tr>`);
        return `${filterbar}<div class="ac-kpis">${kpi("Поставки", fmt(selected.length), "За выбранный период")}${kpi("Товаров в поставках", fmt(totalQuantity), "Штук")}${kpi("Площадка", marketplace === "all" ? "Все" : marketplace === "ozon" ? "Ozon" : "Wildberries", "Текущий фильтр")}</div><div class="ac-grid">${panel("Поставки Ozon / Wildberries", `${selected.length} записей`, table(["Площадка","Поставка","Статус","Склад","Количество","План","Обновлено"], rows, "Поставок за выбранный период нет."), "span-12")}</div>`;
      }

      function renderFinancePage() {
        const gross = metric("gross_sales"), recognized = metric("recognized_sales"), net = metric("net_payout"), margin = metric("contribution_margin");
        const waterfall = [
        ...
        const bars = waterfall.length ? `<div class="ac-waterfall">${waterfall.map(([label,value,tone]) => `<div class="${tone}"><span>${escapeHtml(label)}</span><i class="${safePercentClass(Math.max(5, Math.abs(Number(value)) / max * 100))}"></i><strong>${escapeHtml(money(value))}</strong></div>`).join("")}</div>` : empty("Финансы пока недоступны", "Площадка не предоставила подтверждённые начисления и удержания.");
        const compareRows = financeByMarketplace.map((row) => `<tr><td>${escapeHtml(row.marketplace === "wildberries" ? "Wildberries" : "Ozon")}</td><td>${money(row.recognized_sales)}</td><td>${money(row.deductions)}</td><td>${money(row.net_payout)}</td><td>${fmt(row.return_units)}</td><td>${row.return_amount !== null && row.return_amount !== undefined ? money(row.return_amount) : "—"}</td></tr>`);
        return `${filterbar}<div class="ac-kpis">${kpi("Сумма заказов", safeValue(gross, money), "По ценам товаров", "", comparisonMetric("gross_sales"))}${kpi("Начисления", safeValue(recognized, money), "До удержаний", "", comparisonMetric("recognized_sales"))}${kpi("После удержаний", safeValue(net, money), "К перечислению", "", comparisonMetric("net_payout"))}${kpi("Маржинальный доход", safeValue(margin, money), "Нужна подтверждённая себестоимость")}</div><div class="ac-grid">${panel("Финансовая динамика", period, analyticsHubCombinedChart(chartModels), "span-8")}${panel("Финансовый waterfall", "Только подтверждённые компоненты", bars, "span-4")}${panel("Финансы Ozon / Wildberries", "Начисления, удержания, выплаты и возвраты", table(["Площадка","Начисления","Удержания","После удержаний","Возвраты, шт.","Возвраты, ₽"], compareRows, "Финансовые источники не подключены."), "span-12")}</div>`;
      }
    ...
    function render() {
      ...
      const automaticCompact = Boolean(state.data.is_admin && window.matchMedia && window.matchMedia("(min-width: 900px)").matches);
      const effectiveDensity = state.displayDensity === "auto" ? (automaticCompact ? "compact" : "comfortable") : state.displayDensity;
      document.body.classList.toggle("density-compact", effectiveDensity === "compact");
      document.body.classList.toggle("density-comfortable", effectiveDensity === "comfortable");
    ...
    document.addEventListener("click", (event) => {
      ...
      if (taskAction) {
        ...
        if (action === "all-good" && task) {
          ...
          if (details) details.classList.add("conditional-hidden");
      ...
      if (orderAction) {
        ...
        if (orderAction.dataset.orderAction === "previous-page" || orderAction.dataset.orderAction === "next-page") {
          const direction = orderAction.dataset.orderAction === "next-page" ? 1 : -1;
          state.orderPage = Math.max(0, (Number(state.orderPage) || 0) + direction);
          state.selectedOrder = state.orderPage * ORDER_DOM_WINDOW;
          state.selectedOrderKey = "";
          render();
          return;
        }
      ...
      const wmsTaskPosition = event.target.closest("[data-wms-task-open-allocation]");
      if (wmsTaskPosition) {
        state.wmsShipmentTaskActiveAllocationId = wmsTaskPosition.dataset.wmsTaskOpenAllocation || "";
        state.wmsShipmentTaskLocation = "";
        state.wmsShipmentTaskScannedAllocationId = "";
        state.wmsShipmentTaskExpectedAllocationId = "";
        render();
      ...
      if (wmsTaskAction) {
        ...
        if (action === "back") { state.wmsShipmentTaskDetail = null; state.wmsShipmentTaskLocation = ""; state.wmsShipmentTaskScannedAllocationId = ""; state.wmsShipmentTaskExpectedAllocationId = ""; state.wmsShipmentTaskActiveAllocationId = ""; render(); return; }
        if (action === "back-position") { state.wmsShipmentTaskActiveAllocationId = ""; state.wmsShipmentTaskLocation = ""; state.wmsShipmentTaskScannedAllocationId = ""; state.wmsShipmentTaskExpectedAllocationId = ""; render(); return; }
        if (action === "start") { startWmsShipmentTask(); return; }
        if (action === "confirm") { confirmWmsShipmentTask(); return; }
        if (action === "scan") { scanWms(state.wmsShipmentTaskExpectedAllocationId ? "shipment_product" : "shipment_cell"); return; }
      ...
      if (marketplaceFilterAction) {
        ...
        if (action === "toggle") {
          if (!state.marketplaceFiltersOpen) state.marketplaceFilterSnapshot = {...state.marketplaceFilters};
          state.marketplaceFiltersOpen = !state.marketplaceFiltersOpen;
        }
        if (action === "apply") {
          state.marketplaceFiltersOpen = false;
          state.marketplaceFilterSnapshot = null;
          persistUiState();
        }
        if (action === "cancel") {
          if (state.marketplaceFilterSnapshot) state.marketplaceFilters = {...state.marketplaceFilterSnapshot};
          state.marketplaceFiltersOpen = false;
          state.marketplaceFilterSnapshot = null;
        }
        if (action === "reset") {
      ...
      const chartPoint = event.target.closest("[data-chart-tooltip]");
      if (chartPoint) {
        showMarketplaceChartTooltip(chartPoint);
        return;
      }

      const marketplaceChartMetric = event.target.closest("[data-marketplace-chart-metric]");
      if (marketplaceChartMetric) {
        state.marketplaceChartMetric = marketplaceChartMetric.dataset.marketplaceChartMetric === "net" ? "net" : "revenue";
        persistUiState();
        renderMarketplaces();
        return;
      }
      ...
      if (analyticsOverviewAction) {
        if (analyticsOverviewAction.dataset.analyticsOverviewAction === "refresh") {
          state.analyticsOverview.error = "";
          refreshAnalyticsOverview({force: true});
        }
        return;
      }

      const analyticsMatrixAction = event.target.closest("[data-analytics-matrix-action]");
      if (analyticsMatrixAction) {
        if (analyticsMatrixAction.dataset.analyticsMatrixAction === "links") {
          state.marketplaceProvider = "all";
          state.marketplaceView = "links";
          state.marketplaceDetail = null;
          switchWorkspace("marketplaces");
        } else {
          if (!canManageMarketplaces()) {
      ...
      if (adminSection) {
        ...
        if (state.adminSection === "planning") state.planningResult = null;
        render();
        return;
      }
      ...
      if (adminAction) {
        ...
        if (adminAction.dataset.adminAction === "planning-refresh") refreshAdminPlanning();
        if (adminAction.dataset.adminAction === "planning-calculate") calculateAdminPlanning();
        return;
      }
      ...
      if (profileAction) {
        ...
        if (profileAction.dataset.profileAction === "density") {
          const density = profileAction.dataset.density || "auto";
          state.displayDensity = ["auto", "comfortable", "compact"].includes(density) ? density : "auto";
          persistUiState();
          render();
        }
        return;
      ...
      if (boardOrder && state.data && state.data.is_admin) {
        ...
        const listRows = visibleOrderRows();
        const selectedIndex = Math.max(0, listRows.findIndex((task) => taskIdentity(task) === boardKey));
        state.selectedOrder = selectedIndex;
        state.orderPage = Math.floor(selectedIndex / ORDER_DOM_WINDOW);
        render();
        return;
      }
    ...
    mainButton.addEventListener("click", () => {
      ...
      if (state.screen === "admin") {
        ...
        if (state.adminSection === "planning") { calculateAdminPlanning(); return; }
    ...
    document.addEventListener("input", (event) => {
      ...
      if (routeTask && event.target.closest("#taskGoodQuantity, #taskDefectQuantity, #taskDefectReason, #taskDefectDisposition, #taskDefectComment, #taskPackagingOption")) {
        ...
        if (event.target.id === "taskDefectQuantity") {
          ...
          if (defectDetails) defectDetails.classList.toggle("conditional-hidden", Number(event.target.value || 0) <= 0);
        }
    ...
    document.addEventListener("change", (event) => {
      if (event.target.id === "planningModuleSelect") {
        const allowed = new Set(["mrp", "capacity", "oee", "unit_economics", "forecast", "slotting"]);
        state.planningModule = allowed.has(event.target.value) ? event.target.value : "mrp";
        state.planningResult = null;
        persistUiState();
        render();
        return;
      }
      if (event.target.id === "analyticsMobilePage") {
        const allowed = new Set(["general", "sales", "products", "inventory", "production", "supplies", "finance", "map", "data-quality"]);
        state.analyticsHubTab = allowed.has(event.target.value) ? event.target.value : "general";
        state.marketplaceView = "overview";
        state.marketplaceDetail = null;
        persistUiState();
        render();
      ...
      if (event.target.id === "marketplaceProductPeriod") {
        state.marketplaceProductPeriod = event.target.value || "7d";
        renderMarketplaces();
        return;
      }
      if (["marketplaceDateFrom", "marketplaceDateTo"].includes(event.target.id)) {
        if (event.target.id === "marketplaceDateFrom") state.marketplaceDateFrom = event.target.value || "";
        if (event.target.id === "marketplaceDateTo") state.marketplaceDateTo = event.target.value || "";
    ...
    document.addEventListener("mouseout", (event) => {
      const point = event.target.closest("[data-chart-tooltip]");
      if (point && !point.contains(event.relatedTarget)) hideMarketplaceChartTooltip(point);
      const region = event.target.closest("[data-ac-region-key]");
      if (region && !region.contains(event.relatedTarget)) {
        setAnalyticsRegionHighlight(region.dataset.acRegionKey, false);
      }
    });
    document.addEventListener("focusin", (event) => {
      const point = event.target.closest("[data-chart-tooltip]");
    ...
    document.addEventListener("focusout", (event) => {
      const point = event.target.closest("[data-chart-tooltip]");
      if (point) hideMarketplaceChartTooltip(point);
      const region = event.target.closest("[data-ac-region-key]");
      if (region) setAnalyticsRegionHighlight(region.dataset.acRegionKey, false);
    });
    ...
    function appStateCacheKey(identity) {
      const normalized = String(identity || "").trim();
      return normalized ? `miniapp_bootstrap_state_v${appStateCacheVersion}_${normalized}` : "";
    }

    function clearCachedAppState(identity = "") {
      const candidates = new Set([
        String(identity || "").trim(),
        String(storedWebIdentity || "").trim(),
        String(webSessionProfile.telegram_id || webSessionProfile.username || "").trim(),
      ]);
      candidates.forEach((candidate) => {
        const key = appStateCacheKey(candidate);
        if (!key) return;
        try { window.sessionStorage.removeItem(key); } catch (error) {}
      });
    }

    function cacheAppState(data) {
      if (!isStandaloneWeb || !data || typeof data !== "object" || !data.employee) return false;
      const identity = String(webSessionProfile.telegram_id || webSessionProfile.username || storedWebIdentity || "").trim();
      const employeeIdentity = String(data.employee.telegram_id || "").trim();
      if (!identity || (employeeIdentity && employeeIdentity !== identity)) return false;
      try {
        ...
        if (serialized.length > appStateCacheMaxBytes) return false;
        window.sessionStorage.setItem(appStateCacheKey(identity), serialized);
        return true;
      } catch (error) {
        return false;
      }
    ...
    function restoreCachedAppState() {
      if (!isStandaloneWeb) return false;
      const identity = String(webSessionProfile.telegram_id || webSessionProfile.username || "").trim();
      const key = appStateCacheKey(identity);
      if (!key) return false;
      try {
        const cached = JSON.parse(window.sessionStorage.getItem(key) || "null");
        const age = Date.now() - Number(cached && cached.cached_at || 0);
        const dataIdentity = String(cached && cached.data && cached.data.employee && cached.data.employee.telegram_id || "").trim();
        const valid = cached
          && cached.version === appStateCacheVersion
          && cached.identity === identity
          && cached.data && typeof cached.data === "object"
          && (!dataIdentity || dataIdentity === identity)
          && age >= 0 && age <= appStateCacheMaxAgeMs;
        if (!valid) {
    ...
    function showWebConnection(message, {checking = false, retryDelayMs = 0} = {}) {
      ...
      connectionRetryStatus.textContent = retryDelayMs > 0
        ? `Повторим автоматически через ${formatWaitTime(retryDelayMs / 1000)}`
        : "";
      ...
      webSessionRestorePromise = (async () => {
        ...
        if (result.status === "authenticated") {
          ...
          if (restoredFromCache) {
            // The server verified the session before cached business data became
            // visible. Refresh silently so reload is immediate and then converges
            // to the current database state without blocking the interface.
    ...
    const webVitals = {lcp_ms: 0, inp_ms: 0, cls: 0, long_task_ms: 0};
    function startWebVitalsCollection() {
      ...
      observe("largest-contentful-paint", (entry) => { webVitals.lcp_ms = Math.max(webVitals.lcp_ms, entry.startTime || 0); });
      observe("event", (entry) => { webVitals.inp_ms = Math.max(webVitals.inp_ms, entry.duration || 0); });
      observe("layout-shift", (entry) => { if (!entry.hadRecentInput) webVitals.cls += Number(entry.value || 0); });
      observe("longtask", (entry) => { webVitals.long_task_ms += Number(entry.duration || 0); });
    }
    startWebVitalsCollection();
    window.addEventListener("load", () => {
      window.setTimeout(() => {
        if (!state.data) return;
        api("/api/observability/rum", webVitals).catch(() => {});
      }, 10_000);
    }, {once: true});
