...
${location.code}`)) return;
    ...
    function renderAnalyticsHub() {
      ...
      function renderOverviewPage() {
        ...
        const cards = `<div class="ac-kpis">${kpi("Продано", safeValue(salesUnits), "Товаров за период, шт.")}${kpi("Сумма заказов", safeValue(grossSales, money), "По ценам строк заказа")}${kpi("Заказы", safeValue(orderMetric), "За выбранный период")}${kpi("Остатки", safeValue(stock), "На площадках, шт.")}${kpi("Стоимость остатков", safeValue(stockValue, money), "Остаток × текущая цена")}${kpi("Производство", productionValue, "Факт / план")}</div>`;
        ...
        return `${filterbar}${cards}${sourceState}<div class="ac-grid">${panel("Продажи и финансы", period, analyticsHubCombinedChart(chartModels), "span-8")}${panel("Сравнение площадок", `${providerCards.length} источника`, providerRows || empty("Нет подключённых площадок", "Подключите Ozon или Wildberries в настройках."), "span-4")}${panel("Товарные риски", `${filteredRisks.length} сигналов`, riskRows || empty("Критических рисков нет", "Новых подтверждённых рисков за период не найдено."), "span-4")}${panel("Остатки → производство", `${matrixRows.length} карточек`, matrixBody, "span-4")}${panel("Поставки", `${supplyRows.filter(providerFilter).length} записей`, supplyList || empty("Поставок нет", "За выбранный период поставки не найдены."), "span-4")}${panel("Рекомендации", "На основе рисков", recommendations || empty("Рекомендаций нет", "Данных достаточно или источники пока не сформировали рекомендации."), "span-12")}</div>`;
      }
