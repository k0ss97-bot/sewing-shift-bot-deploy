...
def route_step_component_requirement(route_step: dict):
    operation = route_step.get("operation", "")
    operation_lower = operation.lower()
    preparation_folder = get_preparation_folder(operation)

    if preparation_folder == "Нарезание резинки":
        return "Резинка нужной ширины со склада материалов"
    if preparation_folder == "Нарезание дублерина":
        return "Дублерин нужной ширины со склада материалов"
    if preparation_folder == "Дублирование" or "дублирование" in operation_lower:
        return "Нарезанный дублерин или флизелин"
    if operation == "Сшивание резинок в кольцо":
        return "Нарезанная резинка по размерам"
    if "резин" in operation_lower:
        return "Подготовленная резинка для соответствующего размера"
    if "проклей" in operation_lower:
        return "Нарезанный дублерин"
    if "пуговиц" in operation_lower:
        return "Пуговицы со склада материалов"
    if "кнопок" in operation_lower:
        return "Кнопки со склада материалов"

    return ""


def matrix_component_only_step(route_step: dict):
    operation_lower = route_step.get("operation", "").lower()
    return should_skip_cut_output_step(route_step) or (
        operation_lower.startswith("стачивание ") and "в кольцо" in operation_lower
    )


def matrix_cut_component_requirement(route_step: dict):
    operation_lower = route_step.get("operation", "").lower()
    if not (operation_lower.startswith("стачивание ") and "в кольцо" in operation_lower):
        return ""
    if "горловин" in operation_lower and "манжет" in operation_lower:
        return "Детали горловины, манжет и пояса из готового кроя"
    if "горловин" in operation_lower:
        return "Детали горловины из готового кроя"
    if "манжет" in operation_lower:
        return "Детали манжет и пояса из готового кроя"
    if "пояса" in operation_lower:
        return "Детали пояса из готового кроя"
    return "Детали из готового кроя"


def matrix_prepared_component_requirement(route_step: dict, prepared_components):
    operation_lower = route_step.get("operation", "").lower()
    keywords = []
    if "пришивание пояса" in operation_lower:
        keywords = ["пояс"]
    elif "притачивание горловин" in operation_lower:
        keywords = ["горловин"]
    elif "сборка свитшота" in operation_lower:
        keywords = ["манжет", "пояс"]
    elif "сборка бомбера" in operation_lower:
        keywords = ["манжет", "пояс"]
    elif "резин" in operation_lower and (
        "притач" in operation_lower or "сборка" in operation_lower
    ):
        keywords = ["резин"]

    for component in reversed(prepared_components):
        component_lower = component.lower()
        if keywords and all(keyword in component_lower for keyword in keywords):
            return component
    return ""


def route_step_stock_movement(route_step: dict, step_index: int, step_count: int):
    operation = route_step.get("operation", "")
    preparation_folder = get_preparation_folder(operation)

    if operation == "Формирование готового кроя":
        return "Приход полуфабриката: готовый крой по размеру и цвету"
    if preparation_folder in {"Нарезание резинки", "Нарезание дублерина"}:
        return "Расход материала и приход подготовленного компонента"
    if operation == "Сшивание резинок в кольцо":
        return "Расход нарезанной резинки и приход сшитого компонента"
    if step_index == step_count - 1 or operation == "Размещение на склад":
        return "Приход готовой продукции на склад"
    if step_index == 0:
        return "Передача списанной ткани в раскрой"
    return "Расход предыдущего полуфабриката и приход результата операции"


def matrix_step_stock_movement(
    route_step: dict,
    step_index: int,
    step_count: int,
    component_only_step: bool,
    component_input: str,
):
    if component_only_step and not should_skip_cut_output_step(route_step):
        return "Расход деталей из готового кроя и приход подготовленного компонента"
    if component_input and not component_only_step:
        return "Расход основного полуфабриката и компонента; приход результата операции"
    return route_step_stock_movement(route_step, step_index, step_count)


def operational_matrix_to_dict(product_name: str):
    ...
    matrix_steps = [
        {
            ...
            "operation": "Приход ткани на склад",
            "main_input": "Ткань от поставщика по цветам",
            "component_input": "",
            "output": "Ткань в рулонах на складе материалов",
            "dependency": "Подтверждены материал, цвет и количество рулонов",
            "stock_movement": "Приход ткани: увеличение остатка в рулонах",
            "next_operation": "Создание задания на раскрой",
        },
        {
            "number": 2,
            "route_number": None,
            ...
            "position": "Администратор",
            "operation": "Создание задания на раскрой",
            "main_input": "Месячный план, изделие, размеры и цвета",
            "component_input": "Ткань в рулонах на складе материалов",
            "output": "Задание раскройщику и зарезервированная ткань",
            "dependency": "На складе достаточно рулонов каждого выбранного цвета",
            "stock_movement": "Списание выбранных рулонов со склада материалов",
            "next_operation": route_steps[0]["operation"],
        },
    ]

    primary_output = matrix_steps[-1]["output"]
    previous_step_output = primary_output
    prepared_components = []

    for route_index, route_step in enumerate(route_steps):
        component_only_step = matrix_component_only_step(route_step)
        component_input = (
            matrix_prepared_component_requirement(route_step, prepared_components)
            or route_step_component_requirement(route_step)
            or matrix_cut_component_requirement(route_step)
        )
        dependency = f"Завершён предыдущий этап: {previous_step_output}"
        if component_input:
            dependency += f"; доступен компонент: {component_input}"

        output = route_step["status_after"]
        next_operation = (
            route_steps[route_index + 1]["operation"]
            if route_index + 1 < len(route_steps)
            else "Производственный цикл завершён"
        )
        matrix_steps.append(
            {
                "number": route_index + 3,
                "route_number": route_index + 1,
                "category": route_step_category(route_step),
                "position": route_step["position"],
                "operation": route_step["operation"],
                "main_input": primary_output,
                "component_input": component_input,
                "output": output,
                "dependency": dependency,
                "stock_movement": matrix_step_stock_movement(
                    route_step,
                    route_index,
                    len(route_steps),
                    component_only_step,
        ...
        if component_only_step:
            prepared_components.append(output)
        else:
    ...
    return {
        ...
        "sizes": get_product_sizes(product_name),
        "colors": [format_color_label(color) for color in get_product_colors(product_name)],
        "step_count": len(matrix_steps),
        "steps": matrix_steps,
    }


def get_operational_matrix_catalog():
    return [operational_matrix_to_dict(product_name) for product_name in PRODUCT_ROUTE_MAPS]
...
def get_routes_payload(telegram_id: int):
    return {
        ...
        "operational_matrix": get_operational_matrix_catalog(),
