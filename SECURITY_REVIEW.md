...
| Dependencies | GitHub quality job runs `pip check` and pinned `pip-audit 2.10.1` on Python 3.11 | CONFIGURED; CURRENT CI RESULT PENDING |
| SAST | dedicated SAST result not yet attached | PENDING |
...
3. In-memory IP rate limit сбрасывается при restart и не общий для нескольких backend-процессов.
4. Нет утверждённого срока хранения/анонимизации ФИО, email, phone, IP/user-agent fingerprints и фото брака.

## Gate до релиза

- Получить current CI dependency audit и SAST result.
